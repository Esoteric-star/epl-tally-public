...
Ties are broken first by number of exact scorelines, then alphabetically by name. Predictions lock at kick-off, and that's enforced on the server, not just hidden in the UI so nobody gets clever with the network tab.
